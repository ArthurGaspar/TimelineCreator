...
function saveEvents(events: HistoricalEvent[]): void {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(events));
}

let events: HistoricalEvent[] = loadEvents();
let currentlyEditing: string | null = null;

// DOM
const yearInput = document.getElementById('yearInput') as HTMLInputElement;
const monthInput = document.getElementById('monthInput') as HTMLSelectElement;
...
function setupEditForm(event: HistoricalEvent) {
...
function sortEvents(events: HistoricalEvent[], order: 'asc' | 'desc'): HistoricalEvent[] {
...
function filterEvents(events: HistoricalEvent[], category: string): HistoricalEvent[] {
    if (category === 'all') {
        return events;
    }
...
function renderTimeline() {
    const sortOrder = sortSelect.value as 'asc' | 'desc';
